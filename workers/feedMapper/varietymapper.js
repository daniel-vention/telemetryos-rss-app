/**
 * Variety RSS Feed Mapper
 * 
 * Specialized parser for Variety RSS feeds with enhanced image extraction
 * Handles media:thumbnail and media:content namespace elements and channel logo extraction
 */

/**
 * Parse Variety RSS feed XML and extract articles
 * @param {string} xmlText - Raw RSS XML text
 * @param {string} sourceId - Source feed ID (should be 'variety')
 ...
 */
export function parseVarietyFeed(xmlText, sourceId, sourceLogoUrl) {
  try {
    ...
    if (parseError) {
      throw new Error('Variety RSS parsing error: ' + parseError.textContent)
    }

    // Extract channel image URL if no sourceLogoUrl provided
    // Variety feeds have <channel><image><url> for the feed logo
    ...
    // Media RSS namespace for Variety feeds
    ...
    for (const item of items) {
      try {
        ...
        // Extract article image from Variety RSS patterns
        // Variety uses media:thumbnail and media:content with medium="image"
        let imageUrl = null
        ...
        // Fallback to querySelector for media:content
        if (!imageUrl) {
        ...
        // Try media:content with type starting with "image"
        if (!imageUrl) {
          const mediaContents = item.getElementsByTagNameNS(mediaNS, 'content')
          for (let i = 0; i < mediaContents.length; i++) {
        ...
        console.warn(`Failed to parse Variety RSS item for ${sourceId}:`, itemError)
        continue
      }
    }
    ...
    throw new Error(`Variety RSS feed parsing failed for ${sourceId}: ${error.message}`)
  }
}
